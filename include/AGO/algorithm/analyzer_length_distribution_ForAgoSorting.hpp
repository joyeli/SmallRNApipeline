...
//Joye	void* operator()(int this_analyzer_count, size_t pipe_index, bool eof_flag, int barcode_index=0)
	{
...
//			Analysis (in, -1, -1, 0, "-2",Genome_Table, 0);//Joye
//			Analysis (in, 1, -1, 0, "-2",Genome_Table, 0);//Joye
//Joye			Analysis (in, -1, -1, 0, "-2", 0);
//Joye			Analysis (in, 1, -1, 0, "-2", 0);
...
//Joye		Analysis (in, FilterType::value, DbIndexType::value, DbDepthType::value, boost::mpl::c_str<DbDepthNameType>::value);
		{
			...
			//gOutSet_.resize(this_analyzer_count+1);	//out_set_[] operation on line 379, calloutput will result in segmentation fault
			//MapSum(gOutSet_[this_analyzer_count_], out_set_);
			
			std::lock_guard<std::mutex> lock(gOutMutex_);
			//out_set_[] operation on line 379, calloutput will result in segmentation fault
	 ...
	 * @return void
	 */
	void Analysis2(INPUT_TYPE in, const int filter, const int db_index, const int db_depth, const char* db_depth_name)
	{
		for(auto &anno_rawbed : *in)
		{
			if(this_analyzer_count_ == 0 )
			{
				...
				CalOutput< decltype(anno_rawbed)>(anno_rawbed, 0, -1, 0, 0, "");
			...
			else if(db_index >= 0)
			{
				...
				CalOutput< decltype(anno_rawbed)>(anno_rawbed, 1, filter, db_index, 0, "");
				...
				else if(db_depth >= 0)
				{
					...
					//std::cerr << "TEST " << db_depth_name << "!!" << (std::string(db_depth_name) == "-1") << "!!" << std::endl;
					if(std::string(db_depth_name) == "-1")//bool false
					{
						//std::cerr << "db_index: " << db_index << "\tdb_depth " << db_depth << "\t" << db_depth_name;
						//std::cerr << "\t value " << anno_rawbed.annotation_info_[db_index][db_depth];
						//std::cerr << "\t c_str_value " << anno_rawbed.annotation_info_[db_index][db_depth].c_str() << std::endl;
						//在特定depth下，所有 name 全做
						CalOutput< decltype(anno_rawbed)>
						(anno_rawbed, 2, filter, db_index, db_depth, anno_rawbed.annotation_info_[db_index][db_depth].c_str());
						continue;
					}
					//name 有指定，做特定
				...
				else //depth == -1
				{
					...
					{
						CalOutput< decltype(anno_rawbed)>
						(anno_rawbed, 2, filter, db_index, db_dep, anno_rawbed.annotation_info_[db_index][db_dep].c_str());
					}
			...
			else // db_index == -1, depth all, name all
			{
				...
				for(int db_idx(0); db_idx != anno_rawbed.annotation_info_.size(); ++db_idx)
				{
					...
					CalOutput< decltype(anno_rawbed)>(anno_rawbed, 1, filter, db_idx, 0, "");
					...
					else if(db_depth >= 0)
					{
						if(std::string(db_depth_name) == "-1")//bool false
						{
							//在特定depth下，所有 name 全做
							CalOutput< decltype(anno_rawbed)>
							(anno_rawbed, 2, filter, db_idx, db_depth, anno_rawbed.annotation_info_[db_idx][db_depth].c_str());
							continue;
						}
						//name 有指定，做特定
					...
					else
					{
						...
						{
							CalOutput< decltype(anno_rawbed)>
							(anno_rawbed, 2, filter,db_idx, db_dep, anno_rawbed.annotation_info_[db_idx][db_dep].c_str());
...
//Joye					(anno_rawbed, sys, filter, check_db_idx, check_db_depth, check_depth_name.c_str());
	...
	inline void CalOutput (READ_TYPE &anno_rawbed ,const int sys, const int filter, const int db_index, const int db_depth, const char* db_depth_name, std::map< std::string, std::string >& Genome_Table)//Joyre
...
//		std::get<0> ( out_set_[-1][std::make_tuple(sys,filter,db_index,db_depth,db_depth_name, read_seq)] ) += count;
		std::get<0> ( out_set_[-1][std::make_tuple(sys,filter,db_index,db_depth,"", read_seq)] ) += count; //Joye
...
AnalyzerImpl<INPUT_TYPE, ANALYZER_TYPELIST_GLOBAL_LENDIST, AnalyzerTypes::LengthDistribution>::gOutSet_(0);

template<class INPUT_TYPE>
std::vector< std::string > 
AnalyzerImpl<INPUT_TYPE, ANALYZER_TYPELIST_GLOBAL_LENDIST, AnalyzerTypes::LengthDistribution>::gOutNamePrefix_(0);
