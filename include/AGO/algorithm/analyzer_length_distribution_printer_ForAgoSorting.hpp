#ifndef ANALYZER_PRINTER_HPP_
#define ANALYZER_PRINTER_HPP_
...
template<>
class AnalyzerParameter<AnalyzerTypes::LenDistPrinter>
{
public:
	/** 
...
class AnalyzerImpl<INPUT_TYPE, ANALYZER_TYPELIST, AnalyzerTypes::LenDistPrinter>
{
public:
	/// @brief 此為要使用的 analyzer parameter
	typedef AnalyzerParameter <AnalyzerTypes::LenDistPrinter> AnaPara;
...
//Joye			output_path = std::string("output/sample-") + PipelinePreparator<>::gBarcode_vector_[barcode_index] + std::string("/analysis/");
...
//Joye			output ("ppm", 1);
		...
		{
			AnalyzerImpl<INPUT_TYPE, ANALYZER_TYPELIST_GLOBAL_LENDIST, AnalyzerTypes::LengthDistribution>::gOutSet_.clear();	//Joye
			AnalyzerImpl<INPUT_TYPE, ANALYZER_TYPELIST_GLOBAL_LENDIST, AnalyzerTypes::LengthDistribution>::gOutNamePrefix_.clear();	//Joye
		}
...
//Joye		boost::filesystem::create_directory(dir_output);
...
//Joye	void output(const std::string &unit = "read_count", double normalize_idx = 2)
	{
...
/*
            ...
            file_writter_parameter.bs_file_path_="files?name="+get_filename(dim_z.first, unit + ".tsv")+"&";//text.txt&"
            file_writter_parameter.bs_dir_path_ = "directory="+output_path+"&";
			...
			std::shared_ptr <IoHandlerBaseSpace> out_ (std::make_shared <IoHandlerBaseSpace> (file_writter_parameter));
			vec.push_back (out_);
*/
//			FileWritterBaseSpace out_ (file_writter_parameter);
//			std::ofstream out_ (output_path + get_filename(dim_z.first, unit + ".tsv"));
...
//Joye			std::cout << output_path + get_filename(dim_z.first, unit + ".tsv") << std::endl;
			...
			*(out_) << "--------------------" << std::endl;
			...
			//std::map< std::string, double> all_title;
			...
			for(auto &dim_x : dim_z.second)
			{
				...
				if(count_x == 0)
				{
					std::string LenMir_Name( ".LenDist_" + GMPM + "_" + unit ); //Joye
					std::map< std::string, double > LenMir_Length; //Joye

					*(out_) << "table" << "\t";
					std::string sum_x_name = "SUM_" + get_XYZ_name(Xaxis::first::value);	
					for(auto &dim_y : (dim_z.second)[sum_x_name])
					{
						//if(dim_y.first == "")
							//continue;
						//*(out_) << dim_y.first << "\t";
				...
				std::string LenMir_Name( dim_x.first ); //Joye
				std::map< std::string, double > LenMir_Length; //Joye

				//auto iter_all_title = all_title.begin();
				*(out_) << dim_x.first << "\t";
