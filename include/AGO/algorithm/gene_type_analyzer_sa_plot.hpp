...
class GeneTypeAnalyzerSA_Plot
{
  public:
    ...
    static void output_sa_plot_visualization( const std::string& output_name )
    {
        ...
        output << "        {" << "\n";
        output << "            $Sample = Explode( '-', $TSV );" << "\n";
        output << "            Array_Push( $Sample_List, $Sample[0] );" << "\n";
        output << "        }" << "\n";
        output << "" << "\n";
        ...
        output << "            if( $TSV_File == $Sample_List[$i] ) echo 'selected ';" << "\n";
        ...
        output << "                        $Header[ $i -4 ] = $inFile_Line[$i];" << "\n";
        output << "" << "\n";
        output << "                    $isHeader = false;" << "\n";
        output << "                    continue;" << "\n";
        output << "                }" << "\n";
        output << "" << "\n";
        output << "                if( $Filter != '' && $Filter != 'FilterGMPM' && $inFile_Line[1] < $Filter  )" << "\n";
        output << "                    continue;" << "\n";
        output << "" << "\n";
        ...
        output << "" << "\n";
        output << "                For( $i = 4; $i < Count( $inFile_Line ); ++$i )" << "\n";
        output << "                    $Ratio_Array[ $i -4 ] = $inFile_Line[$i];" << "\n";
        output << "" << "\n";
        ...
        output << "        $LastBin = ( Floor(( Count( $Value_Array ) -1 ) / $isBin ) -1 ) * $isBin;" << "\n";
        output << "" << "\n";
        output << "        For( $i = 0; $i < Count( $Value_Array ); ++$i )" << "\n";
        output << "        {" << "\n";
        output << "            Array_Push( $Temp_Array, $Value_Array[$i] );" << "\n";
        output << "" << "\n";
        output << "            if( $isBin != '' && $isBin != 'isBin' &&   $i > $LastBin    && $i != Count( $Value_Array ) -1 ) continue;" << "\n";
        output << "            if( $isBin == '' || $isBin == 'isBin' || ( $i % $isBin == 0 && $i != 0 ) || $i == Count( $Value_Array ) -1 )" << "\n";
        output << "            {" << "\n";
        output << "                $Sum = 0.0;" << "\n";
        output << "                $SumArray = $Sum_Array;" << "\n";
        output << "                $N = ( $i != Count( $Value_Array ) -1 ? $i : ( $i +1 ));" << "\n";
        output << "" << "\n";
        output << "                For( $j = 0; $j < Count( $Temp_Array ); ++$j )" << "\n";
        output << "                    For( $k = 0; $k < Count( $Temp_Array[$j] ); ++$k )" << "\n";
        output << "                    {" << "\n";
        output << "                        $AllArray[$k] += $Temp_Array[$j][$k];" << "\n";
        output << "                        $SumArray[$k] += $Temp_Array[$j][$k];" << "\n";
        output << "                    }" << "\n";
        output << "" << "\n";
        output << "                For( $j = 0; $j < Count( $SumArray ); ++$j )" << "\n";
        output << "                    $SumArray[$j] = $SumArray[$j] / ( $isBin == '' || $isBin == 'isBin' ? 1 : ( $i % $isBin == 0 ? $isBin : ( $i % $isBin )));" << "\n";
        output << "" << "\n";
        output << "                For( $j = 0; $j < Count( $SumArray ); ++$j )" << "\n";
        output << "                    $Sum += $SumArray[$j];" << "\n";
        output << "" << "\n";
        output << "                For( $j = 0; $j < Count( $SumArray ); ++$j )" << "\n";
        output << "                    $SumArray[$j] = $SumArray[$j] / $Sum;" << "\n";
        output << "" << "\n";
        output << "                $Temp_Array = Array();" << "\n";
        output << "" << "\n";
        output << "                if( $i <= $nNumber )" << "\n";
        output << "                    For( $j = 0; $j < Count( $SumArray ); ++$j )" << "\n";
        output << "                        $L_Array[ $Header[$j] ][$N] = $SumArray[$j];" << "\n";
        output << "" << "\n";
        output << "                if( $i == Count( $Value_Array ) -1 )" << "\n";
        output << "                {" << "\n";
        output << "                    $Sum = 0.0;" << "\n";
        output << "                    $N = Count( $Value_Array );" << "\n";
        output << "" << "\n";
        output << "                    For( $j = 0; $j < Count( $AllArray ); ++$j )" << "\n";
        output << "                        $Sum += $AllArray[$j];" << "\n";
        output << "" << "\n";
        output << "                    For( $j = 0; $j < Count( $AllArray ); ++$j )" << "\n";
        output << "                        $AllArray[$j] = $AllArray[$j] / $Sum;" << "\n";
        output << "" << "\n";
        output << "                    For( $j = 0; $j < Count( $AllArray ); ++$j )" << "\n";
        output << "                        $M_Array[ $Header[$j] ][$N-1] = $AllArray[$j];" << "\n";
        output << "" << "\n";
        output << "                    For( $j = 0; $j < Count( $AllArray ); ++$j )" << "\n";
        output << "                        $M_Array[ $Header[$j] ][$N+1] = $AllArray[$j];" << "\n";
        output << "                }" << "\n";
        output << "" << "\n";
        output << "                if( Count( $Value_Array ) - $i <= $nNumber )" << "\n";
        output << "                    For( $j = 0; $j < Count( $SumArray ); ++$j )" << "\n";
        output << "                        $R_Array[ $Header[$j] ][$N] = $SumArray[$j];" << "\n";
        output << "            }" << "\n";
        output << "        }" << "\n";
        output << "" << "\n";
        ...
        output << "" << "\n";
        output << "        $Count1 = 0;" << "\n";
        output << "        Foreach( $M_Array as $ACGT => $N_Array )" << "\n";
        output << "        {" << "\n";
        output << "            $Count2 = 0;" << "\n";
        output << "" << "\n";
        ...
        output << "                        .color( ['#FF0000', '#0000FF', '#FFBF00', '#088A08', '#6E6E6E'])" << "\n";
