...
class GeneTypeAnalyzerSeedPie
{
  public:
    ...
    static void output_seedpie_visualization( const std::string& output_name )
    {
        ...
        output << "            }" << "\n";
        output << "" << "\n";
        output << "            Array_Push( $Anno_Array, $inFile_Line[0] );" << "\n";
        output << "        }" << "\n";
        output << "" << "\n";
        output << "        echo '<form action='.$_SERVER['PHP_SELF'].' method=post style=display:inline;>';" << "\n";
        ...
        output << "#<!--===================== Read File ======================-->" << "\n";
        output << "" << "\n";
        output << "        $Temp = Tempnam( '/tmp', $TSV_File.'_'.$Annotation_Select.'_'.$isLog );" << "\n";
        ...
        output << "                if( $isLog != '' && $inFile_Line[$i] != 0) $Value = Log( $inFile_Line[$i], $isLog );" << "\n";
